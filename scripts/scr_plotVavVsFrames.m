function scr_plotVavVsFrames(HeaderLines, FrameRate, RefVelocity, RefIntensity, XRange, XPlotType, YPlotType, PlotLegendLabels, PlotColors, PlotMarkers)
% Plotting average Velocity (in a frame) vs frame number (or time)
...
for k = 1:size(fileList, 1)
    ...
    switch(YPlotType)
    	...
    	case 'Velocity'
            yLabelTxt = 'V [mm/s]';
            data(k).Y = (RefVelocity/RefIntensity) * data(k).Y; % intensity to velocity conversion
        otherwise
