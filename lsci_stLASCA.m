function lsci_stLASCA(InputFile, StartFrame, EndFrame, NumericalMethod, PixelXYZ, XYWindowSizePx, ZWindowSizePx, CamExposureTime, WavelengthUm, NA, Magnification)
% Read video, calc Spatio-Temporal Laser Speckle Contrast and save it as multi page tiff file.
...
% NumericalMethod = 'Direct' | 'Sums' | 'SumsVec' | 'ConvFilter' (fastest speed)
% PixelXYZ = [X, Y, Z] (coordinate of the point where we show statistics (K and V)
% XYWindowSizePx = 3 (i.e. 3x3 px), 5 (i.e. 5x5 px) etc (pixel size of the XY sliding window to calc LSC per pixel based on neigbourhood pixel intensities)
...
fprintf('\nStart calculating Spatio-Temporal Laser Speckle Contrast... \n'); % show progress

% Process input and calc Laser Speckle Contrast Map
dataLSP.Contrast = lsciProcessLASCAInputByMethod(inXYZFrames, NumericalMethod, XYWindowSizePx, ZWindowSizePx);
...
baseFileNameLSPCorrelationTime = fullfile(filePath, [fileName '_stLSC-tc']); % Assemble tiff file name for Laser Speckle Correlation Time
baseFileNameLSPVelocity = fullfile(filePath, [fileName '_stLSC-v']); % Assemble tiff file name for Laser Speckle Velocity
...
% Save processed data
saveLspData(dataLSP, InputFile, '_stLSC', PixelXYZ, XYWindowSizePx, ZWindowSizePx, CamExposureTime, NumericalMethod, WavelengthUm, NA, Magnification);
...
function rtrnXYZLSPContrast = lsciProcessLASCAInputByMethod(InXYZFrames, NumericalMethod, XYWindowSizePx, ZWindowSizePx)
% Calc Spatio-Temporal Laser Speckle Contrast Map
% Numerical algorithms take ideas from the following paper:
% W. James Tom et al, "Efficient Processing of Laser Speckle Contrast Images", DOI link: https://doi.org/10.1109/TMI.2008.925081
% Spatio-Temporal Laser Speckle calculation is based on the following paper --> first calc Spatial Contrast in 3x3 window then use ca. 10 frames to further average over temporal window:
% P. Miao et al, "Random process estimator for laser speckle imaging of cerebral blood flow", DOI link: https://doi.org/10.1364/OE.18.000218

% Calc Laser Speckle Contrast map
switch(NumericalMethod)
    case 'Direct' % Slowest speed
        rtrnXYZLSPContrast = stLASCADirect(InXYZFrames, XYWindowSizePx, ZWindowSizePx);
    case 'Sums'
        rtrnXYZLSPContrast = stLASCASums(InXYZFrames, XYWindowSizePx, ZWindowSizePx);
    case 'SumsVec'
        rtrnXYZLSPContrast = stLASCASumsVectorized(InXYZFrames, XYWindowSizePx, ZWindowSizePx);
...
function rtrnXYZLSPContrast = stLASCADirect(InXYZFrames, XYWindowSizePx, ZWindowSizePx)
% Direct calc of Laser Speckle Contrast

[lengthX, lengthY, lengthZ] = size(InXYZFrames);

% Pre-allocate
rtrnXYZLSPContrast = zeros((lengthX - XYWindowSizePx + 1), (lengthY - XYWindowSizePx + 1), (lengthZ - ZWindowSizePx + 1));

% Calc Laser Speckle Contrast map --> k = std(I)/<I>
fprintf('\nProgress LSP Contrast (Direct Calc): 000.0 [%%] | 00000.0 [sec]');
%workerThreads = 4;
%parfor (iZ = 1:(lengthZ - ZWindowSizeFrames + 1), workerThreads) % parallel for-loop through frames
for iZ = 1:(lengthZ - ZWindowSizePx + 1) % loop thorough frames
    ...
    for iX = 1:(lengthX - XYWindowSizePx + 1) % loop through image height
        for iY = 1:(lengthY - XYWindowSizePx + 1) % loop through image width
            subFrames = InXYZFrames(iX:(iX + XYWindowSizePx - 1), iY:(iY + XYWindowSizePx - 1), iZ:(iZ + ZWindowSizePx - 1)); % extract subframe given by the window size
            %meanIntensity = mean(subFrame, [1, 2]); % mean intensity in XY window along Z
            meanIntensity = sum(subFrames, [1, 2])/(XYWindowSizePx^2); % mean intensity in XY window along Z
            stdIntensity = std(subFrames, 0, [1, 2]); % standard deviation in XY window along Z            
            rtrnXYZLSPContrast(iX, iY, iZ) = mean(stdIntensity)/mean(meanIntensity); % calc contrast for the given pixel by avearging over the temporal window
        end
    end
    elapsedTime = toc(startTime);
...
function rtrnXYZLSPContrast = stLASCASums(InXYZFrames, XYWindowSizePx, ZWindowSizePx)
...
for iZ = 1:(lengthZ - ZWindowSizePx + 1) % loop thorough frames
    ...
    for iX = 1:(lengthX - XYWindowSizePx + 1) % loop through image height
        for iY = 1:(lengthY - XYWindowSizePx + 1) % loop through image width
            subFrames = InXYZFrames(iX:(iX + XYWindowSizePx - 1), iY:(iY + XYWindowSizePx - 1), iZ:(iZ + ZWindowSizePx - 1)); % extract subframe given by the window size            
            meanIntensity = sum(subFrames, [1, 2])/(XYWindowSizePx^2); % mean intensity
            meanSqrIntensity = sum(subFrames .^2, [1, 2])/(XYWindowSizePx^2); % mean of squared intensities
            %meanSqrIntensity = sum(subFrame .^2, [1, 2])/(XYWindowSizePx^2 - 1); % mean of squared intensities 
            rtrnXYZLSPContrast(iX, iY, iZ) = sqrt(mean(meanSqrIntensity) - mean(meanIntensity)^2)/mean(meanIntensity); % calc contrast for the given pixel (actually the center pixel of the window)            
        end
    end
    elapsedTime = toc(startTime);
    fprintf('\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b'); % delete previous progress line
...
function rtrnXYZLSPContrast = stLASCASumsVectorized(InXYZFrames, XYWindowSizePx, ZWindowSizePx)
...
% Pre-allocate
rtrnXYZLSPContrast = zeros((lengthX - XYWindowSizePx + 1), (lengthY - XYWindowSizePx + 1), (lengthZ - ZWindowSizePx + 1));
meanIntensityXYZ = zeros((lengthX - XYWindowSizePx + 1), (lengthY - XYWindowSizePx + 1), lengthZ);
meanSqrIntensityXYZ = zeros((lengthX - XYWindowSizePx + 1), (lengthY - XYWindowSizePx + 1), lengthZ);

% Calc Laser Speckle Contrast map --> k = std(I)/<I> = sqrt(<I^2> - <I>^2)/<I> = sqrt(<I^2>/<I>^2 - 1)
fprintf('\nProgress LSP Contrast (SumsVec Calc) (Step 1 of 2): 000.0 [%%] | 00000.0 [sec]');
for iZ = 1:(lengthZ - ZWindowSizePx + 1) % loop throughout frames
    startTime = tic;
    ...
    if (iZ == 1)
        for iX = 1:(lengthX - XYWindowSizePx + 1) % loop through image height            
            ...
            fprintf('(Step 1 of 2): %05.1f [%%] | %07.1f [sec]', (iX/(lengthX - XYWindowSizePx + 1))*100, ((lengthX - XYWindowSizePx + 1) - iX)*(toc(startTime)/iX));
        end
    end
    
    % Average the mean XY intensities over the temporal window Z
    subFramesMeanIntensity = meanIntensityXYZ(:, :, iZ:(iZ + ZWindowSizePx - 1)); % extract XY frame with Z subframe
    meanIntensityZ = sum(subFramesMeanIntensity, 3)./ZWindowSizePx; % mean of the mean XY intensity along Z window
    subFramesMeanSqrIntensity = meanSqrIntensityXYZ(:, :, iZ:(iZ + ZWindowSizePx - 1)); % extract XY frame with Z subframe
    meanSqrIntensityZ = sum(subFramesMeanSqrIntensity, 3)./ZWindowSizePx; % mean of the mean of squared XY intensities along Z
    ...
    elapsedTime = toc(startTime);
    fprintf('\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b'); % delete previous progress line
    fprintf('(Step 2 of 2): %05.1f [%%] | %07.1f [sec]', (iZ/(lengthZ - ZWindowSizePx + 1))*100, ((lengthZ - ZWindowSizePx + 1) - iZ)*elapsedTime);
end

end

function rtrnXYZLSPContrast = stLASCAConvFilter(InXYZFrames, XYWindowSizePx, ZWindowSizePx)
% Conv method (vectorized along XYZ) to calc the Laser Speckle Contrast

[lengthX, lengthY, lengthZ] = size(InXYZFrames);

% Pre-allocate
rtrnXYZLSPContrast = zeros((lengthX - XYWindowSizePx + 1), (lengthY - XYWindowSizePx + 1), (lengthZ - ZWindowSizePx + 1));
meanIntensityXY = zeros((lengthX - XYWindowSizePx + 1), (lengthY - XYWindowSizePx + 1), lengthZ);
meanSqrIntensityXY = zeros((lengthX - XYWindowSizePx + 1), (lengthY - XYWindowSizePx + 1), lengthZ);
%filterKernel1 = ones(XYWindowSizePx, XYWindowSizePx, ZWindowSizePx); % represents 3D sliding window
filterKernel2 = ones(XYWindowSizePx, XYWindowSizePx); % represents 2D sliding window
...
%meanIntensity = convn(InXYZFrames, filterKernel1, 'valid')./(ZWindowSizePx * XYWindowSizePx^2); % mean intensity along Z
%meanSqrIntensity = convn(InXYZFrames .^2, filterKernel1, 'valid')./(ZWindowSizePx * XYWindowSizePx^2); % mean of squared intensities along Z
%rtrnXYZLSPContrast(1:end, 1:end, 1:end) = sqrt(meanSqrIntensity - meanIntensity .^2)./meanIntensity; % calc contrast for the given pixel along Z (actually the center pixel of the window)
...
% Variant 2
% Step 1 --> Precalc mean intensity and mean squared intensity map for each frame (calc only ones)
for iZ = 1:lengthZ % loop through image frames
    startTime = tic;
    
    subFrame = InXYZFrames(:, :, iZ); % extract subframe in XY
    meanIntensityXY(1:end, 1:end, iZ) = filter2(filterKernel2, subFrame, 'valid')./(XYWindowSizePx^2); % mean XY intensity along Z
    meanSqrIntensityXY(1:end, 1:end, iZ) = filter2(filterKernel2, subFrame .^2, 'valid')./(XYWindowSizePx^2); % mean of squared XY intensities along Z
    
    fprintf('\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b'); % delete previous progress line
    fprintf('(Step 1 of 2): %05.1f [%%] | %07.1f [sec]', (iZ/lengthZ)*100, (lengthZ - iZ)*toc(startTime));
end

% Step 2 --> Calc LSP Contrast
for iZ = 1:(lengthZ - ZWindowSizePx + 1) % loop throughout frames
    startTime = tic;
        
    % Average the mean XY intensities over the temporal window Z
    subFramesMeanIntensity = meanIntensityXY(:, :, iZ:(iZ + ZWindowSizePx - 1)); % extract XY frame with Z subframe
    meanIntensityZ = sum(subFramesMeanIntensity, 3)./ZWindowSizePx; % mean of the mean XY intensity along Z window
    subFramesMeanSqrIntensity = meanSqrIntensityXY(:, :, iZ:(iZ + ZWindowSizePx - 1)); % extract XY frame with Z subframe
    meanSqrIntensityZ = sum(subFramesMeanSqrIntensity, 3)./ZWindowSizePx; % mean of the mean of squared XY intensities along Z
    rtrnXYZLSPContrast(1:end, 1:end, iZ) = sqrt(meanSqrIntensityZ - meanIntensityZ .^2)./meanIntensityZ; % calc contrast
...
function saveLspData(DataLSP, InputFile, LASCAMethodString, PixelXYZ, XYWindowSizePx, ZWindowSizePx, CamExposureTime, NumericalMethod, WavelengthUm, NA, Magnification)
