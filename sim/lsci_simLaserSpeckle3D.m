function lsci_simLaserSpeckle3D(ImageWidth, ImageHeight, Frames, ExposureTime, OutputFileType)
...
glLSP.RandomizeUPhaseSwitch = 'off'; % 'on' | 'off' randomize the phase of each Ufield wave to emulate multiple scattering
...
FlowVelocityUm.vx = 3000.0; % in [um/s], flow velocity along X
...
for iZ = 1:Frames
    ...
    %lspChImage = lsci_simGenerateIntensityImage_mex(particles, uint32(chHeightPx), uint32(chWidthPx), LaserWavelengthUm, glLSP.RSCoreLookUpTbl, glLSP.ObservationPlaneXYZUm, glLSP.StepSizeUmLookUpTbl); % the diffraction pattern in the XY plane
    lspChImage = generateIntensityImage(particles, chHeightPx, chWidthPx, LaserWavelengthUm, glLSP.RSCoreLookUpTbl, glLSP.ObservationPlaneXYZUm, glLSP.StepSizeUmLookUpTbl); % the diffraction pattern in the XY plane
...
function rtrnIntensityImage = generateIntensityImage(Particles, ImageHeightPx, ImageWidthPx, LaserWavelengthUm, RSCoreLookUpTbl, ObservationPlaneXYZUm, StepSizeXYZUm)
...
for iSubStep = 1:numSubSteps
    tmpUfiledImage = zeros(ImageHeightPx, ImageWidthPx); % reset
    ...
    parfor i = 1:numberParticles % loop thorough the particles
        ...
        Rz = abs(Particles(i).z0 - ObservationPlaneXYZUm.Z); % not a matrix/vector     
    ...
    rtrnIntensityImage = rtrnIntensityImage + conj(tmpUfiledImage) .* tmpUfiledImage;
