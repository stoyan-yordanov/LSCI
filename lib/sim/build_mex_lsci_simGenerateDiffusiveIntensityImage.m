...
Particles(1).Dx = double(0);
Particles(1).Dy = double(0);
Particles(1).Dz = double(0);
...
% StaticScatters var
StaticScatters = struct([]);
StaticScatters(1).x0 = double(0); % store old X position
StaticScatters(1).y0 = double(0); % store old Y position
StaticScatters(1).z0 = double(0); % store old Z position
    
StaticScatters(1).x = double(0); % new X position
StaticScatters(1).y = double(0); % new Y position
StaticScatters(1).z = double(0); % new Z position
StaticScatters(1).Dx = double(0);
StaticScatters(1).Dy = double(0);
StaticScatters(1).Dz = double(0);
StaticScatters(1).Ufield = complex(1.0); % the init amplitude and phase of the electric field for this particle

StaticScatters(2).x0 = double(0); % store old X position
StaticScatters(2).y0 = double(0); % store old Y position
StaticScatters(2).z0 = double(0); % store old Z position
    
StaticScatters(2).x = double(0); % new X position
StaticScatters(2).y = double(0); % new Y position
StaticScatters(2).z = double(0); % new Z position
StaticScatters(2).Dx = double(0);
StaticScatters(2).Dy = double(0);
StaticScatters(2).Dz = double(0);
StaticScatters(2).Ufield = complex(1.0); % the init amplitude and phase of the electric field for this particle

typeStaticScatters = coder.typeof(StaticScatters, [1 Inf], [false true]);
...
codegen -config:mex lsci_simGenerateDiffusiveIntensityImage -args {typeParticles, typeStaticScatters, typeImageHeightPx, typeImageWidthPx, typeLaserWavelengthUm, typeRSCoreLookUpTbl, typeObservationPlaneXYZUm, typeStepSizeXYZUm} -nargout 1